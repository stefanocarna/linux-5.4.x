...
#define DEFAULT_STATE_SIZE	sizeof(DEFAULT_STATE)
...
/*
 * The idea is to create a proc entry that allows to start/stop the sampling
 * for each thread. A common file is used to register a specific thread to the
 * sampling activity while, a entry with the same pid of the thread will report
 * all files related to detection/sampling activity for that thread.
 *
 * |
 * +--- /proc/detection
 *	|
 ...
 *	|	|
 *	|	+------ state
 *	|	|	(write to disable/enable sampling, read state)
 *	|	|
 *	|	+------ samples
 *	|	|	(write (not allowed), read a list of samples (text))
 * 	|	|
 * 	|	+------ 0123 (sub thread)
 *	|	|	|
 *	|	|	+------ state
 ...
 * 	|
 *	+------ active_threads
 *	|	(write a single PID, read (not allowed))
 *	|
 *	+------ remove_thread
 +	|	(write a single PID (create /PID entry), read (not allowed))
 +	|
 *	+------ sampling_rate
 *	|	(write a sampling rate, read the current sampling rate)
 +	|
 *	+------ debug_level
 *		(write to disable/enable debug_level, read debug_level)
 */
...
static ssize_t active_threads_write(struct file *file,
		const char __user *buffer, size_t count, loff_t *ppos)
{
	int err;
	struct task_struct *ts;
	...
	ts = get_pid_task(find_get_pid(*pidp), PIDTYPE_PID);
	if (!ts) {
		pr_info("Pid %u, no task \n", *pidp);
		goto err;
	}

	dd = init_pid_detection_data(ts);
	if (!dd) {
		pr_info("Pid %u, no dd \n", *pidp);
		goto end;
	}
...
struct file_operations active_threads_proc_fops = {
    .write = active_threads_write,
};
...
static void register_active_threads_proc(void)
{
	...
	proc_create("detection/remove_thread", 0222, NULL,
		    &remove_thread_proc_fops);

	proc_create("detection/active_threads", 0222, NULL,
		    &active_threads_proc_fops);
}

void init_pmc_detection_proc(void)
{
	root_pd_dir = proc_mkdir("detection", NULL);
	if (!root_pd_dir) {
		pr_warning("Cannot create proc entry (root) for pmc_detection\n");
		return;
	}

	spin_lock_init(&hash_lock);

	register_active_threads_proc();
}
